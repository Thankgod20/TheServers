...
func (d darkBlueTheme) Font(style fyne.TextStyle) fyne.Resource {
	// If you have a monospaced TTF you want to embed, replace this with it.
	// For simplicity, we just fallback to the built-in Monospace.
	if style.Monospace {
		return theme.DefaultTheme().Font(style)
	}
	// Force everything to be monospaced:
...
func AggregateHolders(transfers []Transfer) []Holder {
	holderMap := make(map[string]float64)
	for _, t := range transfers {
		holderMap[t.Address] = t.Amount

	}
	var result []Holder
	for addr, amt := range holderMap {
		result = append(result, Holder{Address: addr, Amount: amt})
	}
	return result
}
...
func shortenString(s string) string {
	...
	prefix := s[:8]        // first 8 characters
	suffix := s[len(s)-4:] // last 4 characters
...
func main() {
	...
	// holdersList is a widget.List that displays each holder.
	holdersList := widget.NewList(
		// count
		func() int {
			agg := AggregateHolders(mainChart.transfers) // or however you get holders
			return len(agg)
		},
		// create: return a canvas.Text so we can set .Color
		func() fyne.CanvasObject {
			return canvas.NewText("", color.White)
		},
		// update: populate each row with address -> amount
		func(i widget.ListItemID, co fyne.CanvasObject) {
			agg := AggregateHolders(mainChart.transfers)
			if i >= len(agg) {
				return
			}
			holder := agg[i]
			...
			// If > 0 => show "-> amount" in green
			if math.Abs(holder.Amount) < 0.000001 {
				txt.Text = fmt.Sprintf("%s -> 0", shortenString(holder.Address))
				txt.Color = color.White // or any color you want for zero
			} else {
				txt.Text = fmt.Sprintf("%s -> %.2f", shortenString(holder.Address), holder.Amount)
				txt.Color = color.RGBA{0, 255, 0, 255} // green
			}
			txt.Refresh()
		},
	)

	// Wrap the list in a vertical scroll container
	holdersScroll := container.NewVScroll(holdersList)

	// Use a Border layout so the label is fixed at the top and the list fills the rest.
	holdersPanel := container.NewBorder(
		widget.NewLabel("Holders (Address -> Amount)"), // top
		nil,           // bottom
		nil,           // left
		nil,           // right
		holdersScroll, // center (fills remaining space)
	)
	...
	content := container.NewGridWithColumns(3, leftPanel, holdersPanel, rightPanel)
