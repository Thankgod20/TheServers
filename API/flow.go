...
type FlowAnalyticsService struct {
	...
	jobQueue    chan RefreshJob    // <-- ADD THIS LINE
}

func NewFlowAnalyticsService(redisClient *redis.Client) *FlowAnalyticsService {
	return &FlowAnalyticsService{
		redisClient: redisClient,
		jobQueue:    make(chan RefreshJob, 100),
		// sfGroup is zero-value ready
	}
}

// --- ADD THE ENTIRE WORKER FUNCTION BELOW ---
const MAX_CONCURRENT_FETCHES = 2

func (s *FlowAnalyticsService) StartWorkers() {
	for i := 1; i <= MAX_CONCURRENT_FETCHES; i++ {
		go s.worker(i)
	}
	log.Printf("Started %d background data processing workers", MAX_CONCURRENT_FETCHES)
}
...
// worker is the background process that actually calls computeData.
func (s *FlowAnalyticsService) worker(id int) {
	log.Printf("Worker %d listening for jobs.", id)
	for job := range s.jobQueue {
		log.Printf("Worker %d: processing job for address %s", id, job.Address)

		cacheKey := s.getCacheKey(job.Address, job.Lps)
		redisKey := fmt.Sprintf("flow:%s", cacheKey)

		// This block is almost identical to your singleflight logic,
		// but it's now running in a controlled, sequential worker.
		computedData, err := s.computeData(context.Background(), job.Address, job.Lps)
		if err != nil {
			log.Printf("Worker %d: ERROR computing data for %s: %v", id, job.Address, err)
			continue // Skip to the next job
		}

		finalJSON, err := json.Marshal(computedData)
		if err != nil {
			log.Printf("Worker %d: ERROR marshalling data for %s: %v", id, job.Address, err)
			continue
		}

		compressedData, err := compress(finalJSON)
		if err != nil {
			log.Printf("Worker %d: ERROR compressing data for %s: %v", id, job.Address, err)
			continue
		}

		err = s.redisClient.Set(redisKey, compressedData, CACHE_TTL).Err()
		if err != nil {
			log.Printf("Worker %d: ERROR setting Redis cache for %s: %v", id, job.Address, err)
		}

		log.Printf("Worker %d: finished and cached job for address %s", id, job.Address)
		runtime.GC() // Suggest a garbage collection cycle
	}
}
...
*/
// Replace the old triggerBackgroundRefresh with this one.
func (s *FlowAnalyticsService) triggerBackgroundRefresh(ctx context.Context, address, redisKey string, lps []string) {
	// The singleflight group here now serves a different purpose: to prevent
	// adding the *same exact job* to the queue multiple times in quick succession.
	// The `_refresh` key ensures it doesn't collide with the main computation singleflight.
	s.sfGroup.Do(redisKey+"_refresh", func() (interface{}, error) {
		log.Printf("Adding refresh job for %s to the queue.", redisKey)

		// Create the job and send it to the channel.
		// This is non-blocking if the channel buffer has space.
		job := RefreshJob{
			Address: address,
			Lps:     lps,
		}
		...
		select {
		case s.jobQueue <- job:
			log.Printf("Successfully queued job for %s.", redisKey)
		default:
			log.Printf("Warning: Job queue is full. Dropping refresh request for %s.", redisKey)
		}

		// We return immediately. The actual work happens in the background worker.
		// The singleflight 'Forget' should happen after a short duration to prevent re-queueing too fast.
		time.AfterFunc(1*time.Minute, func() {
			s.sfGroup.Forget(redisKey + "_refresh")
		})
