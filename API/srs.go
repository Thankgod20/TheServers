...
*/
// REFACTORED: streamAndComputeAggregates to use a two-pass approach
func streamAndComputeAggregates(jsonData io.Reader, lpsFilter []string, intervalMinutes int) (*ComputedDataType, error) {
	// ---- Pass 1: Discover categories and unique timestamps ----

	// Create a temporary reader for the first pass
	var pass1Data bytes.Buffer
	teeReader := io.TeeReader(jsonData, &pass1Data)

	addressMax := make(map[string]float64)
	timestampsSet := make(map[string]bool)

	decoder1 := json.NewDecoder(teeReader)
	// Consume stream tokens: { "holders" [
	if _, err := decoder1.Token(); err != nil {
		return nil, err
	}
	if _, err := decoder1.Token(); err != nil {
		return nil, err
	...
	for decoder1.More() {
		var holder HolderHistoryEntry
		if err := decoder1.Decode(&holder); err != nil {
			return nil, fmt.Errorf("pass 1 decoding error: %w", err)
		}
		currentAddressMax := addressMax[holder.Address]
		for i, ts := range holder.Time {
			if roundedTimeStr, err := roundTimeToInterval(ts, intervalMinutes); err == nil {
				timestampsSet[roundedTimeStr] = true
			}
			amount := holder.Amount[i]
			if amount > currentAddressMax {
				currentAddressMax = amount
	...
	// Create the canonical sorted timeline
	allTimestamps := make([]string, 0, len(timestampsSet))
	for ts := range timestampsSet {
		allTimestamps = append(allTimestamps, ts)
	}
	sort.Strings(allTimestamps)

	// Create final data structures with pre-allocated maps
	holdingsResult := CategoryHoldings{Whales: make(HoldingsMap), Retail: make(HoldingsMap), Lps: make(HoldingsMap)}
	countsResult := CategoryHolderCounts{Whales: make(CountsMap), Retail: make(CountsMap), Lps: make(CountsMap)}
	for _, t := range allTimestamps {
		holdingsResult.Whales[t], holdingsResult.Retail[t], holdingsResult.Lps[t] = 0.0, 0.0, 0.0
		countsResult.Whales[t], countsResult.Retail[t], countsResult.Lps[t] = 0, 0, 0
	}

	// ---- Pass 2: Populate the final structures ----
	decoder2 := json.NewDecoder(&pass1Data)
	// Consume stream tokens again: { "holders" [
	if _, err := decoder2.Token(); err != nil {
		return nil, err
	}
	if _, err := decoder2.Token(); err != nil {
		return nil, err
	...
	for decoder2.More() {
		var holder HolderHistoryEntry
		if err := decoder2.Decode(&holder); err != nil {
			return nil, fmt.Errorf("pass 2 decoding error: %w", err)
		}

		// Determine category once per holder
		var category string
		if whales[holder.Address] {
			category = "whale"
		} else if retail[holder.Address] {
			category = "retail"
		} else if lpsActual[holder.Address] {
			category = "lp"
		} else {
			continue
		}
		...
		for i, ts := range holder.Time {
			...
			amount := holder.Amount[i]
			if amount > 0 { // Only count active holders
				switch category {
				case "whale":
					holdingsResult.Whales[roundedTimeStr] += amount
					countsResult.Whales[roundedTimeStr]++
				case "retail":
					holdingsResult.Retail[roundedTimeStr] += amount
					countsResult.Retail[roundedTimeStr]++
				case "lp":
					holdingsResult.Lps[roundedTimeStr] += amount
					countsResult.Lps[roundedTimeStr]++
				}
			}
		}
	}
...
func (s *SrsService) HandleSrsRequest(w http.ResponseWriter, r *http.Request) {
	...
	cacheEntryStr, redisErr := s.redisClient.Get(redisStoreKey).Result()
	if redisErr == nil {
		log.Println("SRS serving stale cache for", redisStoreKey)

		// Check last refresh timestamp
		shouldRefresh := false
		if val, ok := s.lastRefresh.Load(redisStoreKey); ok {
		...
		if shouldRefresh {
			log.Println("Triggering background SRS refresh for", redisStoreKey)
			s.lastRefresh.Store(redisStoreKey, time.Now())
			s.triggerSrsBackgroundRefresh(ctx, paramCacheKey, address, redisStoreKey, lps)
		}
